...
void drawButton(GtkButton *button, gpointer dd_Info)
{
    ...
    g_print("Selected: %d", selected);

    switch (selected)
    {
    case 0:
        gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(data->drawArea), drawSineWave, NULL, NULL);
        break;
    case 1:
        gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(data->drawArea), drawCosineWave, NULL, NULL);
        break;
    case 2:
        gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(data->drawArea), drawLinear, NULL, NULL);
        break;
    }
}
...
void app_activate(GApplication *app, GFile **files, int n_files, char *hint)
{
    ...
    boxSideMenu = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
